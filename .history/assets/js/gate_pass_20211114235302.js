...
idList = [idCounter];
let data = [];
let gp_number_manual,
  gate_pass_date,
  gate_pass_party_id,
  gate_pass_party_name,
  gate_pass_type,
  gate_pass_grand_total,
  gate_pass_contact = "";
...
function addSelectItems(arrayNames, arrayId, $select) {
  ...
  for (var i = 0; i < arrayNames.length; i++) {
    $select.append(
      $("<option></option>").attr("value", arrayId[i]).text(arrayNames[i])
    );
  }
}
      ...
      .then((data) => {
        if (data.status == "ok") {
          ...
          window.location.replace(
            "/gate_pass/view-gate-pass/" + data.gp_number
          );
        } else if (data.status == "error") {
          toastr.error("Error: " + data.errorMessage);
...
function getGatePass() {
  ...
  gate_pass_type = document.querySelector(
    'input[name="gate_pass_type_in_out"]:checked'
  ).value;
  gate_pass_contact = document.getElementById("gate_pass_contact").value;
  gate_pass_grand_total = document.getElementById(
    "gate_pass_grand_total"
  ).value;
  ...
  if (gate_pass_party_id == "") {
    ...
    document.getElementById("gate_pass_name_error").innerHTML = "";
    document.getElementById("gp_number_manual_error").innerHTML =
      "Enter Manual GP Number";
    return false;
  } else {
    document.getElementById("gate_pass_name_error").innerHTML = "";
    let gate_pass_commodity,
      gate_pass_unit,
      gate_pass_unit_amount,
      gate_pass_details;
    for (let i = 0; i <= idList.length; i++) {
      ...
      gate_pass_commodity = document.getElementById(
        "gate_pass_commodity" + i
      ).value;
      gate_pass_quantity = document.getElementById(
        "gate_pass_quantity" + i
      ).value;
      gate_pass_unit = document.getElementById("gate_pass_unit" + i).value;
      gate_pass_unit_amount = document.getElementById(
        "gate_pass_unit_amount" + i
      ).value;
      gate_pass_total_amount = document.getElementById(
        "gate_pass_total_amount" + i
      ).value;
      gate_pass_details = document.getElementById(
        "gate_pass_details" + i
      ).value;
...
function getGrandTotal() {
  ...
  for (var i = 0; i < idList.length; i++) {
    let inputTotalAmount = document.getElementById(
      "gate_pass_total_amount" + i
    ).value;
...
function getCashVoucher() {
  ...
  cash_voucher_signature = document.getElementById(
    "cash_voucher_signature"
  ).value;
  ...
  if (cash_voucher_type == "") {
    ...
    if (cash_voucher_type == "Pay") {
      cash_voucher_type = "Credit";
    } else if (cash_voucher_type == "Receive") {
      cash_voucher_type = "Debit";
    }
...
function buttonXgpRowDelete() {
  ...
  document
    .getElementById("tab2")
    .removeChild(document.getElementById("gp_row0"));
}
